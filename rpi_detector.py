...
CAMERA_URL = "http://192.168.42.129:8080"
SENSOR_DISTANCE = 2.0
IDLE_TIMEOUT = 5.0
K = 3
SAMPLE_DT = 0.05
...
def send_to_server(entry_area: str, exit_area: str, speed_kmph: float, nameplate: str, char_confidences: list[float], image_path: str):
    url = f"{SERVER_URL}/api/detection"
    data = {
        "entry_area": entry_area,
        "exit_area": exit_area,
...
def main():
    alpr = init_alpr()
    vehicle_model = None if NO_VEHICLE_FILTER else init_vehicle_detector()
    require_vehicle = (vehicle_model is not None) and (not NO_VEHICLE_FILTER)
